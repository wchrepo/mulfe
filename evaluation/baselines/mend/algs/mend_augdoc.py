...
from ..hooks import hook_model,hook_model_context
from ..utils import _inner_params, _logits, tokenize_prefix_and_target
...
class MEND(EditableModel):
    ...
    def clean_factors(self):
        for n, p in _inner_params(
            ...
            if hasattr(p,"__x__"):
                del p.__x__
            if hasattr(p,"__delta__"):
                del p.__delta__

    def __init__(self, model, config, model_constructor, mend=None, edit_lrs=None,tokenizer=None):
    ...
    def optimizer_groups(self):
        return [{"params":self.mend.parameters(),'lr':self.config.transform_lr},
                {"params":[self.edit_lrs],'lr':self.config.lr_lr}]
    def edit(
        self,
        batch,
        condition=None,
        ...
        #     cur_loss.backward()
        # loss = torch.tensor(0.,device=self.config.edevice)
        if self.config.mend.doc_loss_coeff > 0.:
            doc_batch = [e if isinstance(e,str) else e['edit'] for e in batch ]
            doc_batch = self.tokenizer(doc_batch,padding=True, return_tensors="pt")
            doc_batch['labels'] = doc_batch["input_ids"].masked_fill(
                doc_batch["input_ids"] == self.tokenizer.pad_token_id, -100)
            doc_batch = doc_batch.to(device=self.config.device)
            with hook_model_context(self.model, self.config.model.inner_params):
                doc_outputs = _logits(self.model(**doc_batch))
                doc_loss = self.config.mend.doc_loss_coeff * self.edit_loss_fn(doc_outputs, doc_batch["labels"])["nll"] * len(batch)
                doc_loss.backward()
            # loss = loss + doc_loss
        if self.config.mend.aug_loss_coeff > 0:
            prefixes = [p['input'] for e in batch for p in e['aug_pairs']]
            targets = [p['target'] for e in batch for p in e['aug_pairs']]
            if targets:
                aug_batch = tokenize_prefix_and_target(self.tokenizer,prefixes,targets)
                aug_batch = aug_batch.to(device=self.config.device)
                if self.config.mend.aug_loss_type == 'nll':
                    with hook_model_context(self.model, self.config.model.inner_params):
                        aug_outputs = _logits(self.model(**aug_batch))
                        aug_loss = self.config.mend.aug_loss_coeff * self.edit_loss_fn(aug_outputs, aug_batch["labels"])["nll"]
                        aug_loss.backward()
                elif self.config.mend.aug_loss_type == 'ict_distill':
                    ict_prefixes = [e['edit']+"\n\n"+p['input'] for e in batch for p in e['aug_pairs']]
                    ict_batch = tokenize_prefix_and_target(self.tokenizer,ict_prefixes,targets).to(device=self.config.device)
                    with torch.no_grad():    
                        ict_outputs = _logits(self.model(**ict_batch))[:,:-1] [ict_batch['labels'][:,1:]!=-100]
                    with hook_model_context(self.model, self.config.model.inner_params):
                        aug_outputs = _logits(self.model(**aug_batch))
                        aug_outputs = aug_outputs[:,:-1][aug_batch['labels'][:,1:]!=-100]
                        ict_outputs = ict_outputs + self.config.mend.ict_contra_coeff*(ict_outputs-aug_outputs)
                        #contrastive实际上没有多大用处？相当于放大差异项？也不尽然，会改变pre分布
                        aug_loss = self.config.mend.aug_loss_coeff * kl_loc_loss(ict_outputs.detach(),aug_outputs) * len(batch)
                        aug_loss.backward()
            # loss = loss + aug_loss
        # loss.backward()
