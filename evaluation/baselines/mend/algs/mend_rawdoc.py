...
class MEND(EditableModel):
    ...
    def edit(
        ...
        batch['labels'] = batch["input_ids"].masked_fill(
            batch["input_ids"] == self.tokenizer.pad_token_id, -100)
        batch = batch.to(device=self.config.device)
        cur_outputs = _logits(self.model(**batch))
        cur_loss = self.edit_loss_fn(cur_outputs, batch["labels"])["nll"]
        cur_loss.backward()
        ...
        if scale_withbatchsize:
            scale_coeff = len(batch)
        else:
            scale_coeff = 1
        new_params = []
        for n, p in edited_model.named_parameters():
            if n in pset:
                new_params.append(p - updates[n]/scale_coeff)
            else:
                new_params.append(p)
